            ...
            var Preloader = new Phaser.Class({
                ...
                preload: function () {
                    this.add.image(0, 0, 'imgStar0tBG').setOrigin(0);
                    ...
                    for (var i = 1; i <= 9; i++) {
                        this.load.image('imgM_balloon' + i, 'assets/sprites/balloons/' + i + '.png');
                    }

                    for (var i = 1; i <= 6; i++) {
                        this.load.image('imgM_balloonL' + i, 'assets/sprites/balloons/l.' + i + '.png');
                    }
            ...
            var IntroScene = new Phaser.Class({
                ...
                initialize:
                    ...
                    function IntroScene() {
                        Phaser.Scene.call(this, { key: 'IntroScene', active: true });
                    },
                preload: function ()
                {
                    this.load.image('imgStar0tBG', 'assets/background/start0.jpg');

                },
                create: function () {
                    this.add.image(0, 0, 'imgStar0tBG').setOrigin(0);
                    timedEvent = this.time.addEvent({ delay: 1250, callback: () => { this.scene.start('preloader'); }, callbackScope: this, repeat: 0, startAt: 0 });
                }
            ...
            var StartScene = new Phaser.Class({
                ...
                initialize:
                    ...
                    function StartScene() {
                        Phaser.Scene.call(this, { key: 'StartScene', active: false });
                    },
            ...
            var config = {
                ...
                scene: [IntroScene, Preloader, FinalScene, MagazineScene, LaptopScene, Laptop2Scene, Laptop3Scene, DeskScene, ShelfScene, BoardScene, MapScene, BedScene, TreadmillScene, SportsbagScene, WardrobeScene, MainSceneR, MainScene0, StartScene, MarathonScene],
            ...
            var finalLink = 'https://adventurerooms.ch/';
            var depth = 100;
            var tap = 0;
            var printerCount;
            var fileL1;
            ...
            function createFinalScene() {
                this.add.image(0, 0, 'imgFinal').setOrigin(0);
                this.sound.play('mp3Final');

                this.input.on('pointerdown', function (pointer, gameObject) {

                    this.cameras.main.fadeOut(1000);
                    timedEvent = this.time.addEvent({ delay: 1250, callback: () => { window.open(finalLink); }, callbackScope: this, repeat: 0, startAt: 0 });
            ...
            function createMarathonScene() {
                ...
                timedEvent = this.time.addEvent({
                    delay: 304000, callback: () => {
                        for (i = 0; i < 100; i++) {
                            scaleBalloons[i] = Phaser.Math.Between(150, 175);
                            balloon = this.physics.add.sprite(Phaser.Math.Between(2000, 2900), Phaser.Math.Between(700, 900),
                                'imgM_balloon' + Phaser.Math.Between(1, 9)).setScale(1 - 100 / scaleBalloons[i])
                                .setDepth(scaleBalloons[i] + 2000)
                                .setVelocityX(- Phaser.Math.Between(30, 50))
                                .setVelocityY(- Phaser.Math.Between(10, 15));
                        }

                        for (i = 1; i <= 6; i++) {
                            scaleBalloons[i] = Phaser.Math.Between(175, 225);
                            balloon = this.physics.add.sprite(2250 + i * 75, 700 + i * 20,
                                'imgM_balloonL' + i).setScale(1 - 100 / scaleBalloons[i])
                                .setDepth(scaleBalloons[i] + 2200)
                                .setVelocityX(- Phaser.Math.Between(60 - i * 3, 75 - i * 2))
                                .setVelocityY(- Phaser.Math.Between(20 - i * 3, 25 - i * 2));
                        }
                    }, callbackScope: this, repeat: 0, startAt: 0
                });
